...
4. **Manifest path:** if the plan has a `## Ticket Manifest` section, materialize it: create `tickets/`, write one queue line and one `T-###.md` per manifest row. The queue line carries every field the schema above requires — `id`, `title`, `priority`, `deps`, `territory`, `verify` and `verify_expect`, plus `repo` whenever the work lands outside `<project>` — alongside `status`, `attempts`, `origin` and the claim fields at their initial values. Take each from the manifest row that carries it, and finish `verify` and `verify_expect` in steps 5b and 5c; a field the manifest never named is derived here, not invented later by whoever adjudicates the ticket. The `T-###.md` takes its Trace, Acceptance criteria, Territory and Verify command from that same row, with the other sections as empty headings. The manifest was approved with the plan — no second approval.
...
- The rewrite step that turns a terse status line into a spoken briefing shells out to the `claude` binary, and the OGG rewrap shells out to `ffmpeg`. Both are **optional** and the note still goes out without either, but they fail differently and only one of them says so. The rewrite names its reason on stderr every way it can fall back — binary missing, timeout, non-zero exit, empty output, a rewrite that failed the briefing checks — and sends your message verbatim. The `ffmpeg` path is **silent**: `ensure_ogg_opus` returns no bytes when the binary is absent or the conversion fails, delivery falls through to a plain text message, and the only signal is the route printed on stdout (`delivered: text` rather than `delivered: voice`).
